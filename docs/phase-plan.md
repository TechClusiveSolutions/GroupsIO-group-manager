...
* **Further amendment, 2026-08-12** (see PRD section 2.1 and `subgroup-crud-and-admin-pages-design.md` sections 11 and 15): a fourth "Plugin Configuration" page allowing an admin to set the parent group when `GROUPS_IO_PARENT_GROUP` is not defined in `wp-config.php`; an activation-time admin notice pointing to that page when no parent group is configured by either means; and an "add member" control on Subgroup Management's Details view, searching existing tracked members rather than requiring the member-first flow through User Assignment.
* **Further amendment, 2026-08-12 (#50/#51)** (see `subgroup-crud-and-admin-pages-design.md` section 8): Subgroup Management's Create/Update/Delete move from a single synchronous read-back check to a queued, retried background check (new `SubgroupExecutionEngine`) - an immediate "submitted" response, up to 3 retried attempts, a persistent dismissible notice only on final failure, and `AuditLog::record()` now called for every attempt (previously never called for these three actions at all). The live integration test is also extended to cover `update_subgroup()` against the real test group, not just mocked responses.
* Exit criterion: an admin can create, list, update, and delete Groups.io subgroups directly from the Subgroup Management page (**changed 2026-08-12**: each action's Groups.io write and its confirmation now complete asynchronously via `SubgroupExecutionEngine`, not a single synchronous read-back); can manually add or remove an individual member's subgroup access from the User Assignment page, with the action recorded as a sticky override and in the audit log; and the Feature Controls page (relocated Phase 1 settings) is reachable from the same "GroupsIO Management" menu — all verified with a screen reader pass, per `CLAUDE.md`'s Accessibility by Design section. **Additional criterion, 2026-08-12 amendment**: an admin can reach and use the Plugin Configuration page (read-only when the constant is set, editable when it isn't), the activation notice correctly appears/disappears based on configuration state, and an admin can add an existing tracked member to a subgroup directly from that subgroup's Details view — all verified with a screen reader pass. **Additional criterion, 2026-08-12 amendment (#50/#51)**: a genuine Create/Update/Delete failure surfaces as a persistent, dismissible notice rather than a one-shot redirect notice; a real success within the retry budget produces no extra notice; and `update_subgroup()`'s rename/title/desc behavior is confirmed against the real test group by the extended integration test.
...
* **Added 2026-08-12**: a real uninstall/data-cleanup routine (`uninstall.php` / `register_uninstall_hook()`) dropping the plugin's own tables and options on deletion. None exists today - deactivating or deleting the plugin currently leaves `bits_groupsio_member_index`, `bits_groupsio_audit`, and every plugin option in the database untouched. Explicitly deferred rather than built alongside the Phase 3 amendment above, since it's a genuinely separate, destructive feature needing its own design pass (e.g., unconditional wipe on delete vs. an explicit opt-in setting) - tracked here as a design item, not v1-blocking. Until built, `docs/index.md`'s uninstall documentation describes the current no-cleanup behavior as-is.
