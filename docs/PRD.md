...
* ~~Confirm WordPress.com Business Plan hosting supports everything this design assumes~~ — resolved against WordPress.com's own documentation (Business/Commerce plans run on "Atomic" infrastructure): custom, arbitrarily-sourced plugin installation is supported (via SFTP/SSH or upload — an Atomic-specific capability beyond lower plans); `wp-config.php` is directly editable via SFTP (core files and default themes/plugins are locked, `wp-config.php` is not); custom database tables via `$wpdb`/`dbDelta` carry no plan-specific restriction, since Atomic-hosted sites are effectively full WordPress installs; Action Scheduler is a standard `wp_cron`-extending library with no plan-specific incompatibility; the staging site feature is confirmed included, and WordPress.com automatically adds a `WP_ENVIRONMENT_TYPE=staging` constant to the staging site's `wp-config.php` (see the new note in section 3.1 on using this). This was confirmed against WordPress.com's general published documentation, not by direct access to the live BITS site — a hands-on check against the actual site remains worthwhile once available, but is not a blocker for proceeding.
...
All Phase 0 verification items are now resolved.
