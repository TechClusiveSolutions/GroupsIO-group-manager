...
* **Scope**: the GroupsIO Management menu structure (`admin-menu.spec.ts`); the Feature Controls page, load/edit/save/persist (`feature-controls.spec.ts`); Subgroup Management's full List/Create/Details lifecycle across all three views (`subgroup-management.spec.ts`); User Assignment's List view (`user-assignment-list.spec.ts`); and User Assignment's Details/Add Groups state-changing actions — Remove Selected (including the parent-group inline confirmation and its cancel path), Add Selected, Clear override, the group-owner parent-removal safeguard, and the "Sync" manual-processing control present on every GroupsIO Management view (`user-assignment-actions.spec.ts`). At least one test (`tests/e2e/tests/end-to-end.spec.ts`) exercises the complete path from an unauthenticated login through to a Groups.io update, rather than reusing a pre-authenticated session like the other specs.
